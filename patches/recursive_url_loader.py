...
class RecursiveUrlLoader(BaseLoader):
    ...
    def _get_sub_links(self, raw_html: str, base_url: str) -> List[str]:
        ...
        for link in all_links:
            ...
            # Remove the fragment (e.g. anchor ''#) components from the URL.
    ...
    def _get_child_links_recursive(
        ...
        if self.exclude_dirs and any(
            url.endswith(exclude_dir) for exclude_dir in self.exclude_dirs
        ):
            return []
        ...
        if depth == 0:
            ...
            loaded_link = Document(
                ...
                metadata=self._gen_metadata(text, url),
            )
            yield loaded_link
        ...
        for link in absolute_paths:
            ...
            if link not in visited:
                ...
                if self.exclude_dirs and any(
                    link.endswith(exclude_dir) for exclude_dir in self.exclude_dirs
                ):
                    continue
                
                try:
                    print(f"_get_child_links_recursive url: {link}")
